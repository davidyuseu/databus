...
public class ReadCellData<T> extends CellData<T> {
    ...
    public static excel.metadata.data.ReadCellData<?> newEmptyInstance() {
        return newEmptyInstance(null, null);
    }

    public static excel.metadata.data.ReadCellData<?> newEmptyInstance(Integer rowIndex, Integer columnIndex) {
        excel.metadata.data.ReadCellData<?> cellData = new excel.metadata.data.ReadCellData<>(CellDataTypeEnum.EMPTY);
        cellData.setRowIndex(rowIndex);
        cellData.setColumnIndex(columnIndex);
        return cellData;
    }

    public static excel.metadata.data.ReadCellData<?> newInstance(Boolean booleanValue) {
        return newInstance(booleanValue, null, null);
    }

    public static excel.metadata.data.ReadCellData<?> newInstance(Boolean booleanValue, Integer rowIndex, Integer columnIndex) {
        excel.metadata.data.ReadCellData<?> cellData = new excel.metadata.data.ReadCellData<>(booleanValue);
        cellData.setRowIndex(rowIndex);
        cellData.setColumnIndex(columnIndex);
        return cellData;
    }

    public static excel.metadata.data.ReadCellData<?> newInstance(String stringValue, Integer rowIndex, Integer columnIndex) {
        excel.metadata.data.ReadCellData<?> cellData = new excel.metadata.data.ReadCellData<>(stringValue);
        cellData.setRowIndex(rowIndex);
        cellData.setColumnIndex(columnIndex);
        return cellData;
    }

    public static excel.metadata.data.ReadCellData<?> newInstance(BigDecimal numberValue, Integer rowIndex, Integer columnIndex) {
        excel.metadata.data.ReadCellData<?> cellData = new excel.metadata.data.ReadCellData<>(numberValue);
        cellData.setRowIndex(rowIndex);
        cellData.setColumnIndex(columnIndex);
        return cellData;
    }
    ...
    @Override
    public excel.metadata.data.ReadCellData<Object> clone() {
