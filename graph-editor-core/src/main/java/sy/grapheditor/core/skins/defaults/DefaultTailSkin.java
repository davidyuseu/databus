 ...
 */
package sy.grapheditor.core.skins.defaults;
...
import sy.grapheditor.core.skins.defaults.tail.RectangularPathCreator;
import sy.grapheditor.core.skins.defaults.utils.ArrowUtils;
...
public class DefaultTailSkin extends GTailSkin {
    ...
    protected final Group group = new Group(line, endpoint, arrow);

    private static final double OFFSET_DISTANCE = 5;
...
//        drawArrow(start, end, false);
     ...
     */
    private void drawArrow(final Point2D start, final Point2D end, boolean ifSmart) {

        if (getItem().getType().contains("output")) {
            ArrowUtils.drawHeadLorR(arrow, start, end, ifSmart, OFFSET_DISTANCE);
        } else {
            ArrowUtils.drawHeadLorR(arrow, end, start, ifSmart, OFFSET_DISTANCE);
        }
    }
