...
import sy.grapheditor.core.connections.ConnectionCopier;
 ...
 * 复制的规则如下：
 * <ol>
 * <li>复制选定的所有节点及其连接器 (containment references)。</li>
 * <li>如果一个连接的起始节点和目标节点被复制了，则该连接和该连接的关节都将复制</li>
 * </ol>
 * </p>
 */
public class SelectionCopier {
    ...
    public void copy() {
        ...
        for (final GNode node : model.getNodes()) {
            if (selectionManager.isSelected(node)) {
                ...
                node.setProcessorJson(ProcessorSJsonUtil.objToStr(TitledGNodeAttachmentUtil.getProcessor(node)));
    ...
    private void saveParentPositionInScene() {
        ...
        if (!selectionManager.getSelectedItems().isEmpty()
                && !selectionManager.getSelectedNodes().isEmpty()) {
